...
bool cEnsemble::get_param (char *param, char *string_received)
{
	...
	//- comment to avoid spoofing and perf issue
...
//-----------------------------------------------------------
bool cEnsemble::axis_calibration_is_active (void)
{
	int status = 0;
	if (!get_axis_status (status))
		return false;
	std::bitset<32> bs_status(status);
	//return (status & 0x0800)!=0 ;
	return bs_status.test(11); //- 11th bit
}
...
//-----------------------------------------------------------
//- get_axis_velocity_feedback
//-----------------------------------------------------------
bool cEnsemble::get_axis_velocity_feedback (double &velocity_vfbk)
{
	char s[SIZE_BUFFER];
	char string_received[SIZE_BUFFER];
	sprintf (s, "VFBK %s \n", this->axis_name.c_str ());
	bool ok = send_receive_and_test(s,string_received);
	sscanf(string_received,"%lf",&velocity_vfbk);
	return ok;
}

//-----------------------------------------------------------
//- get_axis_velocity_command
//-----------------------------------------------------------
bool cEnsemble::get_axis_velocity_command (double &velocity_vcmd)
{
	char s[SIZE_BUFFER];
	char string_received[SIZE_BUFFER];
	sprintf (s, "VCMD %s \n", this->axis_name.c_str ());
	bool ok = send_receive_and_test(s,string_received);
	sscanf(string_received,"%lf",&velocity_vcmd);
	return ok;
}

//-----------------------------------------------------------
//- set_axis_velocity
//- Set the velocity by doing a null movement + store it as the default speed (used at reboot)
//-----------------------------------------------------------
bool cEnsemble::set_axis_velocity (double velocity)
{
	char s[SIZE_BUFFER];
	char sformat[SIZE_BUFFER];
	sprintf (s, "%lf", velocity);
    //- first set as default speed
	set_param(const_cast <char*> ("DefaultSpeed"), s); //- Default velocity is set when the controller reboot
	
    //- then do a null movement to set the velocity (it seems to be the only way)
	// send a "MOVEINC Axis F velocity" -
...
bool cEnsemble::commit_parameters ()
{
	...
	send_receive_and_test (s, string_received);
	int fault_status = -1;
	sscanf (string_received, "%d", &fault_status);
	if (fault_status != 0)
		return false;
