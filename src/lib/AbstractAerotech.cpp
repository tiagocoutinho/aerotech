...
bool AbstractAerotech::send_and_receive (char *command, char *received)
{
	...
	bool ok = send_and_receive (std::string (command), resp);
...
//-----------------------------------------------------------
bool AbstractAerotech::set_wait_mode (const std::string& wait_mode)
{
	if (! is_ready_to_accept_cmd  ())
		return false;
	char s[SIZE_BUFFER];
	//- NOWAIT , MOVEDONE,  INPOS (move done+in position)
	sprintf (s, "WAIT MODE %s\n", wait_mode.c_str());
